...
class MLMSim(torch.nn.Module):
    ...
    def get_vector_list(self, question, all_answers):
       with torch.no_grad():
            ...
            all_examples_in_cluster = self.model(torch.LongTensor(padded_ids))[0].cpu().data.numpy()
    ...
    def train_models(self, question_string, all_predicted_answers, true_answers):
        ...
        for cid, each_cluster in enumerate(true_answers):
            ...
            for word in each_cluster:
                seen_words[word] = cid
        starttime = time.time()
        vectorized_answers = self.get_vector_list(question_string, all_answers+all_predicted_answers)
        ...
        with torch.no_grad():
            ...
            for each_group in list(set(answer_list)):
                ...
                y = numpy.array([float(int(bool(x==each_group))) for x in answer_list])
            ...
            for each_group in sorted(predictors):
                ...
                for each_example in range(number_of_test_examples):
                    predictions_dictionary[each_example] = predictions_dictionary.get(each_example, []) + [(p[each_example], each_group)]
            ...
            for example in predictions_dictionary:
                ...
                if all_predicted_answers[example] in seen_words:
                    ...
                    if best[0] > self.threshold:
                        ...
                        selected = -1
                self.mapping_log.write("\t".join([question_string, all_predicted_answers[example], str(selected), "_".join(cluster_lists.get(selected, []))])+"\n")
            return scoring_map
