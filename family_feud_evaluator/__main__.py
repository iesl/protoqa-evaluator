...
def main():
    ...
    convert_parser_sp = convert_parser.add_subparsers(help='Type of conversion', dest='type')

    clustered_data_parser = convert_parser_sp.add_parser('clustered', help='Convert clustering XLSX files to JSONL')
    ...
    ranking_data_parser = convert_parser_sp.add_parser('ranking', help='Convert ranking XLSX files to JSONL')
...
def convert_clustered(config):
    ...
    q = dict()
    for xlsx_file in config['xlsx_files']:
        next_q = load_data_from_excel(xlsx_file, len(q))
        assert set(q.keys()).intersection(next_q.keys()) == set()
        q.update(next_q)
...
def convert_ranking(config):
    ...
    answers_dict = convert_ranking_data_to_answers(ranking_data, question_data)
    if not config['include_do_not_use']:
        ...
        print(f'Removing {num_int} answers whose associated questions were marked as DO_NOT_USE')
