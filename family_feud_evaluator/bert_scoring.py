...
class TransformerScoringModel:
    def __init__(self, model_type = 'bert', model_name_or_path = 'bert-base-cased', no_cuda = True, seed = 42):
        np.random.seed(seed)
    ...
    def __call__(self, question, answer):
        ...
        representation = encoded_layers[0, question_length:question_length + answer_length, :].mean(dim=0)
        return representation

    def preprocessing(self, q: Dict[str, Union[str, Dict]], answers: List[str]) -> Tuple[Dict[str, Union[str, Dict]], List[str]]:
    ...
    def _preprocess_pred_answers(self, question: str, answers: List[str]) -> List[torch.Tensor]:
        return [self(question, ans) for ans in answers]

    def _preprocess_true_answers(self, q: Dict[str, Union[str, Dict]]) -> Dict[str, Union[str, Dict]]:
        q = deepcopy(q)
...
@torch.no_grad()
def cosine_similarity_score(pred_answer: torch.Tensor, true_answer: torch.Tensor) -> float:
    return ((F.cosine_similarity(pred_answer, true_answer, 0) + 1)/2).item()


def tensor_cluster_score_func(pred_answers: List[str],
        ...
        if isinstance(true_ans, frozenset):
            score_func = partial(all_pairs_scores, score_func=score_func, reduction_func=cluster_reduction_func, preprocess_func=lambda z: [z] if isinstance(z,torch.Tensor) else z)
        return all_pairs_scores(pred_answers, true_answers, score_func)


hard_bert_eval = partial(general_eval, score_func=cosine_similarity_score, cluster_score_func=tensor_cluster_score_func, string_preprocessing=lambda z: z, score_matrix_transformation=np.round)
...
def main():
    ...
    original_question = "Name something that is hard to guess about a person you are just meeting."
    ...
    # take the average of the answer embeddings.
    representation1 = encoded_layers[0, question_length:question_length + ans1_length, :].mean(dim=0)
    representation2 = encoded_layers[1, question_length:question_length + ans2_length, :].mean(dim=0)
