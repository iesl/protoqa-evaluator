...
from family_feud_evaluator.scoring import longest_common_subsequence_score, longest_common_substring_score, \
    wordnet_score, bleu_score, meteor_score, exact_match
...
fast_money_bleu_sim = partial(fast_money, score_func=bleu_score)
fast_money_meteor_sim = partial(fast_money, score_func=meteor_score)

EVAL_METHODS = [("Fast Money", fast_money), ("Family Feud", family_feud), ("Set Intersection", set_intersection), ("Hard Set Intersection", hard_set_intersection), ("Family Feud (5 incorrect)", family_feud_5_incorrect)]
...
def calc_scores(predictions: Dict[str, List[str]], question_data: Dict) -> float:
    ...
    for eval_method in EVAL_METHODS:
        ...
        for sim_func in SIM_FUNCS:
            ...
            method = partial(eval_method[1], score_func=sim_func[1])
